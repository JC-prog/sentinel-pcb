...
called directly by a route (POST /api/agents/explainability-review in app/chat/api/chat.py).
Nothing else calls it in-process: agents don't import each other (tests/chat/test_agent_boundaries.py).
...
board_id/component_ref/package/feature), which is why it needs app.chat.uploads.resolve_upload_path
...
from app.chat.agents.case_agent.graph import PCBInspectionState, get_pipeline
