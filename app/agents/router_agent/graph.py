...
currently on offer (app/agents/registry.py's ToolRegistry.specs(), already filtered by
...
Same sync-OpenAI-client-call shape as app/agents/weather_agent/graph.py's _query_advisory_llm -
...
(rather than a plain function) to match every other agent in app/agents/ - state TypedDict, a
...
def _query_router_llm(user_prompt: str) -> dict[str, Any]:
    ...
    # (app/agents/weather_agent/graph.py, app/agents/explainability_review_agent/models.py) -
